...
function asScript(templateDir, options) {
  var fileGlob, helpers, namespace, terminating = false, workerFns = [], workers = {};
  ...
  fileGlob = path.resolve(templateDir, '**/*.hbs');
  ...
  function createWorker(partial, cb) {
    var bufs = [], handlebarsArgs = [], output, partialName, worker;

    if (terminating) {
      return cb(new Error('process terminating'));
    }

    partialName = partialPathToName(partial);
